---
name: loopany-health-check
description: Weekly workspace integrity pass — run `loopany doctor`, sweep overdue followups, and force closure on stuck artifacts (running tasks, active signals, pending proposals). Use on weekly cadence, separate from the daily [[./followups.md]] skill.
---

# health-check — weekly drift + stuck-artifact sweep
...
2. **Slippage** — items whose `check_at` slid past today, never
   addressed. Daily [[./followups.md]] stays focused on today; this is
...
superseded`, etc. Use [[./task-lifecycle.md]] for tasks.

**C. Shouldn't have had a `check_at`.** Remove the date with a brief
note. Repeated mis-scheduling is fodder for [[./improve.md]].
...
- **Real, going to act** → upgrade to task via [[./signal-capture.md]]
- **Real, not acting** → dismiss with reason (so [[./improve.md]] can
  see it if it recurs)
...
[[./followups.md]]**: every surfaced item ends this session in
...
platforms, make sure [[./improve.md]] is on this week's cron.
...
session didn't happen yet." Daily health-check becomes a second
followups digest and trains the user to ignore both.
...
FEED:     ≥3 resolutions → nudge improve
```
